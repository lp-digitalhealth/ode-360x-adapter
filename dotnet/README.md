...
tests/Ode.Adapter.Tests/    dependency-free console test runner
```
...
Host endpoints: `POST /360x/inbound`, `POST /ode/task-event`, `GET /plugins`,
`GET /episodes`, `GET /healthz`. Runs in dry-run by default (no FHIR server needed);
set `ODE_ADAPTER_DRY_RUN=false` and `ODE_ADAPTER_ODE_BASE_URL` to drive a real server,
and `ODE_ADAPTER_FHIR_BACKEND=onyx` to select the OnyxOS backend.
